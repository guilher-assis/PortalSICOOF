...
switch($acao){
	...
	case 'cadastrar':{
		...
		$date = date('Y-m-d');
		$timestamp2 = strtotime('+9 day', strtotime($date));
		$date_exp= date('Y/m/d', $timestamp2);
            $cadastrar = new Cadastrar();	
		$cadastrar->setNome($_POST['nome']);
		$cadastrar->setCodigo($_POST['codigo']);
		$cadastrar->setTipo($_POST['tipo']);
		$cadastrar->setCenarios($_POST['cenarios']);
	$cadastrar->setDataInicio($date);
	$cadastrar->setDataFim($date_exp);
	...
	case 'editar_demanda':{
		...
		$demanda->setNome($_POST['nome_editar']);
		$demanda->setCodigo($_POST['codigo_editar']);
		$demanda->setCenarios($_POST['cenarios_editar']);
		$demanda->setTipo($_POST['tipo']);
		...
		//Passaremos para o m�todo de autentica��o da DAO um objeto da classe Usu�rio. Armazenaremos na vari�vel $resultado o que este m�todo retornar. 
		$resultado = $painelDAO->listaDemanda();
		//Agora chamamos um m�todo da View passando para o mesmo o que foi retornado pela DAO.
		...
		$demanda->setNome($_POST['pesquisa_demanda']);
